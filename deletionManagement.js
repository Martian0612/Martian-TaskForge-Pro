...
//         }
//         else {
//             console.log("Task lists exists!");
//             taskListItems.forEach(listItem => {
//                 listItem.classList.add("selection-active");  
//                 console.log("Added .selection-active to:", listItem);
//                 // const selectionArea = card.querySelector('.task-selection');
//                 // selectionArea.style.display = 'flex';
//             });
//         }
...
//                     // const selectionArea = card.querySelector('.task-selection');
//                     // selectionArea.style.display = 'none';
//                     listItem.classList.remove("selection-active");
//                 });                
//             }
//             // taskCards.forEach(card => {
//             //     // const selectionArea = card.querySelector('.task-selection');
...
// List view version deletion code
export class DeletionManager {
    constructor() {
        this.selectedTasks = new Set();
        this.taskToDelete;
    ...
    showAllCheckboxes() {
        ...
        // Process card view items if they exist
        if (taskCards.length > 0) {
            console.log("Task cards exist!");
            taskCards.forEach(card => {
                card.classList.add("selection-active");  
                console.log("Added .selection-active to card:", card);
                console.log("showing checkboxes for all task cards.");
            });
        }
        
        // Process list view items if they exist
        if (taskListItems.length > 0) {
            console.log("Task list items exist!");
            taskListItems.forEach(listItem => {
                listItem.classList.add("selection-active");  
                console.log("Added .selection-active to list item:", listItem);
                console.log("showing checkboxes for all task list items.");
            });
        }
    ...
    hideAllCheckboxes() {
        if (this.selectedTasks.size === 0) {
            ...
            if (taskCards.length > 0) {
                taskCards.forEach(card => {
                    card.classList.remove("selection-active");
                    console.log("Removed .selection-active from card:", card);
                });
            }
            ...
            if (taskListItems.length > 0) {
                taskListItems.forEach(listItem => {
                    listItem.classList.remove("selection-active");
                    console.log("Removed .selection-active from list item:", listItem);
                });                
            }
    ...
    handleCheckboxChange(event) {
        ...
        // Check if event is an event object or a DOM element
        let taskId, isChecked;
        
        if (event instanceof Event) {
            // It's an actual event
            event.stopPropagation();
            const checkbox = event.target;
            taskId = checkbox.getAttribute('data-task-id');
            isChecked = checkbox.checked;
        } else {
            // It's a DOM element (from the event listener in app.js)
            const checkbox = event.target;
            taskId = checkbox.getAttribute('data-task-id');
            isChecked = checkbox.checked;
        }
        
        console.log("taskId:", taskId, "isChecked:", isChecked);
        console.log("Selected tasks before change:", this.selectedTasks);
        console.log("Selection bar display:", this.selectionBar.style.display);
