    ...
    // ----- LIST VIEW -----
        ...
        tasksToDisplay.forEach(task => {
            if (!document.querySelector(`[data-task-id="${task.task_id}"]`)) {
                ...
                let formattedDate;
                const dueDate = new Date(task.dueDate);
                console.log("dueDate in list view is ", dueDate);
                if (dueDate === 'Thu Jan 01 1970 05:30:00 GMT+0530 (India Standard Time)') {
                    formattedDate = 'No due Date';

                }
                else {
                    formattedDate = dueDate.toLocaleDateString() + ' ' + dueDate.toLocaleTimeString();
                }
    ...
    function handleTaskDisplayClick(event) {
        ...
        if (viewMode === "card") {
            ...
            if (event.target.matches('.reminder-button') || event.target.closest('.reminder-button')) {
                ...
                console.log("bell task id is ", taskId);
                console.log('bell state is ', bellState);
                // const task = currentUser.taskList.find(t => t.task_id === taskId);
                ...
                if (bellState === "noDueDate") {
                    ...
                    if (storedUsers) {
                        ...
                        if (currentUserData && currentUserData.taskList) {
                            const task = currentUserData.taskList.find(t => t.task_id === taskId);
                            if (!task) {
                                console.error("No matching task found. Possibly stale ID.");
                                return;
                            }
                            console.log("task name is ", task.task);
                            console.log("Task for edit (user switch):", task);
                            populateModalForm(task);
                            modal.style.display = "block";
...
async function handleNotificationSubmit(event) {
    ...
    if (existingNotification) {
        ...
        await updateBellIcon(currentReminderTaskId);
...
async function handleTaskSubmit(event) {
    ...
    try {
        ...
        if (isEditMode && editTaskId) {
            ...
            if (currentUser.updateTask(editTaskId, formData)) {
                // Always reassign updated currentUser back to userMap and localStorage
                userMap.set(currentUser.username, currentUser);
                localStorage.setItem(userListKey, JSON.stringify(Array.from(userMap.values().map(user => user.toData()))));
                showSuccess('Task updated successfully');
                modal.style.display = "none";


                if (formData.dueDate === "null" || formData.dueDate === null) {
                    // await dbStorageInstance.deleteNotificationByTaskId(editTaskId);
                    // console.log("Deleted notification due to dueDate being cleared.");
                    // updateBellIcon(editTaskId);

                    await removeTaskReminder(editTaskId);
                    await updateBellIcon(editTaskId);
