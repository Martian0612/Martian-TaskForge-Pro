...
//             console.log("Container HTML inserted!");
//             taskDisplay.innerHTML = `
...
//         if (!cardContainer) {
//             throw new Error("cardContainer is null after HTML insert!");
//         }
...
//             // Clear the display area first
//             taskDisplay.innerHTML = "";
//             console.log(" LIST Container HTML inserted!");
//             // Initial render
//             taskDisplay.innerHTML = `
...
//         if (!listContainer) {
//             throw new Error("listContainer is null after HTML insert!");
//         }
...
/**
 * Renders tasks in either card or list view.
 * - On full render (append: false): clears and recreates all containers before rendering.
 * - On append (append: true): keeps containers and only appends new tasks.
 *
 * @param {object} user - The current user object.
 * @param {Array} tasksToShow - Array of task objects to render.
 * @param {string} message - Optional message to show if no tasks.
 * @param {string} viewMode - "card" or "list".
 * @param {string|null} focusedTaskId
...
async function displayTasks(
    ...
    // 1. Setup/clear containers ONLY if not appending
    if (!append) {
        if (viewMode === "card") {
            taskDisplay.innerHTML = `
                <div class="task-container"></div>
                <div class="load-more-container">
                    <button class="load-more-button">Load More Tasks</button>
                    <div class="loading-spinner"><span class="material-icons">autorenew</span></div>
                </div>
            `;
        } else {
            taskDisplay.innerHTML = `
    ...
    // 2. Get the correct container for this view
    const cardContainer = viewMode === "card"
        ? taskDisplay.querySelector('.task-container')
        : null;
    const listContainer = viewMode === "list"
        ? taskDisplay.querySelector('.task-list-container')
        : null;

    // 3. Throw an error if the container is missing (should never happen now)
    if (viewMode === "card" && !cardContainer) {
        throw new Error("cardContainer is null! The container must exist before rendering tasks.");
    }
    if (viewMode === "list" && !listContainer) {
        throw new Error("listContainer is null! The container must exist before rendering tasks.");
    }

    // 4. Handle "no tasks" message
    if (message) {
        taskDisplay.innerHTML = `<div class="no-tasks-container"><p class="no-tasks-message">${message}</p></div>`;
        return;
    }
    ...
    for (const task of tasksToShow) {
        // Only add if the card/list item doesn't exist yet (prevent duplicates on append)
        if (
            (viewMode === "card" && !document.getElementById(`task-${task.task_id}`)) ||
            (viewMode === "list" && !document.getElementById(`task-list-${task.task_id}`))
        ) {
            if (viewMode === "card") {
                const taskCard = document.createElement("div");
                taskCard.className = "task-card";
                taskCard.dataset.taskId = task.task_id;
                taskCard.id = `task-${task.task_id}`;
                // FILL OUT YOUR TASK CARD HTML BELOW
                const dueDate = new Date(task.dueDate);
                const formattedDate = dueDate.toLocaleString();
                // If you have a reminder status async function, you can await it:
                // const hasReminder = await checkExistingNotification(task.task_id);
                taskCard.innerHTML = `
                    <div class="task-selection">
                        <div class="task-checkbox-wrapper">
                ...
                // If you have tag rendering, reminders, etc., call those here (async if needed)
                // updateTaskCardTags(task, userListKey, userMap);
    ...
    // 6. Optional: Highlight the focused task
    if (focusedTaskId) {
        setTimeout(() => {
            const elem = document.getElementById(
                viewMode === "card"
                ? `task-${focusedTaskId}`
                : `task-list-${focusedTaskId}`
            );
            if (elem) {
                elem.scrollIntoView({ behavior: "smooth", block: "center" });
                highlightTask(focusedTaskId); // Your highlight logic here
            }
        }, 200);
    }

    // 7. Setup event handlers if needed (checkbox, delete, etc.)

    // 8. Setup "Load More" handler ONLY in card view, and ONLY if not appending
    if (!append && viewMode === "card") {
        ...
        if (loadMoreBtn && loadingSpinner) {
            loadMoreBtn.addEventListener('click', async () => {
                ...
                const start = appState.pagination.tasksPerPage * (appState.pagination.currentPage - 1);
                const end = appState.pagination.tasksPerPage * appState.pagination.currentPage;
                const newBatch = allSortedTasks.slice(start, end);
                displayTasks(user, newBatch, "", "card", null, { append: true });
                loadMoreBtn.disabled = false;
                loadingSpinner.classList.remove('visible');
            });
        }
...
async function handleTemporaryTaskOnLoadMore() {
    ...
    // Get all tasks in their natural order
    const allSortedTasks = sorting();
...
//     // -- Only for CARD VIEW and only if focusedTaskId is present --
//     if (
//         appState.currentView === "card" &&
//         focusedTaskId
//     ) {
...
//         if (!taskInCurrentBatch) {
//             // Not in batch: append temp
//             const focusedTask = sortedAndFilteredTasks.find(task => task.task_id === focusedTaskId);
//             if (focusedTask) {
//                 appState.temporaryTask = { taskId: focusedTaskId, isActive: true };
//                 // Pass the combined batch to displayTasks
//                 displayTasks(currentUser, [...currentBatchTasks, focusedTask], "", appState.currentView, focusedTaskId);
//                 return;
//             }
//         } 
//         // else {
//         //     // If it IS in the batch, clear temp state
//         //     cleanupTemporaryTask();
//         // }
//     }
...
/**
 * Handles batch logic, view mode, and calls displayTasks with the correct options.
 * - Always does a full re-render unless specifically appending (on "Load More").
 */
function refreshTaskDisplay(focusedTaskId = null) {
    if (!currentUser || !currentUser.taskList) return;

    const sortedAndFilteredTasks = sorting(); // Your sorting/filtering logic
    let batchToShow = sortedAndFilteredTasks;

    // Handle jump-to-task/temporary batch logic (for card view)
    if (appState.currentView === "card" && focusedTaskId) {
        const currentBatchEnd = appState.pagination.tasksPerPage * appState.pagination.currentPage;
        const currentBatchTasks = sortedAndFilteredTasks.slice(0, currentBatchEnd);
        const taskInCurrentBatch = currentBatchTasks.find(task => task.task_id === focusedTaskId);
        ...
        if (!taskInCurrentBatch) {
            const focusedTask = sortedAndFilteredTasks.find(task => task.task_id === focusedTaskId);
            if (focusedTask) {
                batchToShow = [...currentBatchTasks, focusedTask];
                appState.temporaryTask = { taskId: focusedTaskId, isActive: true };
            }
        } else {
            cleanupTemporaryTask();
            batchToShow = currentBatchTasks;
        }
    } else if (appState.currentView === "card") {
        // Normal card view, show current page's batch (no jump)
        const end = appState.pagination.tasksPerPage * appState.pagination.currentPage;
        batchToShow = sortedAndFilteredTasks.slice(0, end);
        cleanupTemporaryTask();
    } else if (appState.currentView === "list") {
        // In list view, you may want to show all tasks or a paginated batch (adapt as needed)
        batchToShow = sortedAndFilteredTasks; // Or paginate if you want
        cleanupTemporaryTask();
    }
    ...
    if (batchToShow.length === 0) {
        appState.pagination.currentPage = 1;
        appState.pagination.allTasksLoaded = true;
        if (appState.searchTerm) message = "No tasks match your search.";
        else if (Object.values(appState.activeFilters).some(filters => filters.length > 0)) message = "No tasks match your filters.";
        else message = "No tasks available.";
    }

    // Always do a full re-render here (append: false)
    displayTasks(currentUser, batchToShow, message, appState.currentView, focusedTaskId, { append: false });
}
